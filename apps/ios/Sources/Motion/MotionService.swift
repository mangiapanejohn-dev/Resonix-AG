...
final class MotionService: MotionServicing {
    func activities(params: ResonixMotionActivityParams) async throws -> ResonixMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[ResonixMotionActivityEntry], Error>) in
        ...
        return ResonixMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: ResonixPedometerParams) async throws -> ResonixPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<ResonixPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = ResonixPedometerPayload(
