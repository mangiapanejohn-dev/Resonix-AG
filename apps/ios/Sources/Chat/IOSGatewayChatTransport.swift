...
struct IOSGatewayChatTransport: ResonixChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.resonix", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> ResonixChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ResonixChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ResonixChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ResonixChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ResonixChatAttachmentPayload]) async throws -> ResonixChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ResonixChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(ResonixChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(ResonixGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ResonixGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ResonixChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ResonixAgentEventPayload.self)
                        {
