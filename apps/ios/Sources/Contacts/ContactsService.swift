...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ResonixContactsSearchParams) async throws -> ResonixContactsSearchPayload {
        ...
        return ResonixContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ResonixContactsAddParams) async throws -> ResonixContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ResonixContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ResonixContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ResonixContactPayload {
        ResonixContactPayload(
