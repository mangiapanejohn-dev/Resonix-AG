...
final class RemindersService: RemindersServicing {
    func list(params: ResonixRemindersListParams) async throws -> ResonixRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[ResonixReminderPayload], Error>) in
        ...
        return ResonixRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ResonixRemindersAddParams) async throws -> ResonixRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ResonixReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ResonixRemindersAddPayload(reminder: payload)
    }
