...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ResonixProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ResonixProtocol.AnyCodable] = [
            "presence": .init([ResonixProtocol.AnyCodable(entry)]),
        ]
        let payload = ResonixProtocol.AnyCodable(payloadMap)
