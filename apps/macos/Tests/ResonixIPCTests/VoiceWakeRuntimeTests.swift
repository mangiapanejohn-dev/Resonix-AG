...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["resonix"]
        let text = "hey resonix"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey resonix do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["resonix"], minPostTriggerGap: 0.3)
