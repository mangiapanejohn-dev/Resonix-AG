...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.resonix", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ResonixPeekabooBridgeServices?
    private static var resonixSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Resonix", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ResonixPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.resonixSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.resonixSocketPath, privacy: .public)")
    }
...
@MainActor
private final class ResonixPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.resonix.peekaboo")
