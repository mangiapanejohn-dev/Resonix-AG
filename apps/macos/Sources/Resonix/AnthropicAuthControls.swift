...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ResonixOAuthStore.AnthropicOAuthStatus = ResonixOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ResonixOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ResonixOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ResonixOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ResonixOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Resonix can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ResonixOAuthStore.AnthropicOAuthStatus,
