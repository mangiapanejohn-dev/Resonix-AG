...
private let webChatSwiftLogger = Logger(subsystem: "ai.resonix", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ResonixChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ResonixChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ResonixChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ResonixChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ResonixGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ResonixChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ResonixChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ResonixChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ResonixChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ResonixChatView>) -> NSViewController
    {
