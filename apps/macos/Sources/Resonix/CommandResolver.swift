...
enum CommandResolver {
    private static let projectRootDefaultsKey = "resonix.gatewayProjectRootPath"
    private static let helperName = "resonix"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let resonixEntry = root.appendingPathComponent("resonix.mjs").path
        if FileManager().isReadableFile(atPath: resonixEntry) { return resonixEntry }
        let binEntry = root.appendingPathComponent("bin/resonix.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/resonix")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let resonixPaths = self.resonixManagedPaths(home: home)
        if !resonixPaths.isEmpty {
            extras.insert(contentsOf: resonixPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + resonixPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func resonixManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".resonix"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func resonixExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectResonixExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("resonix.mjs").path,
            root.appendingPathComponent("bin/resonix.js").path,
        ]
    ...
    static func hasAnyResonixInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.resonixExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func resonixNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let resonixPath = self.projectResonixExecutable(projectRoot: root) {
                return [resonixPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "resonix", subcommand] + extraArgs
            }
            if let resonixPath = self.resonixExecutable(searchPaths: searchPaths) {
                return [resonixPath, subcommand] + extraArgs
            }

            let missingEntry = """
            resonix entrypoint missing (looked for dist/index.js or resonix.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real resonix CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/resonix"
        ...
        \(cliSection)
        \(projectSection)
        if command -v resonix >/dev/null 2>&1; then
          CLI="$(command -v resonix)"
          resonix \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/resonix.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/resonix.mjs"
            node "$PRJ/resonix.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/resonix.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/resonix.js"
            node "$PRJ/bin/resonix.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent resonix"
          pnpm --silent resonix \(quotedArgs);
        else
          echo "resonix CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ResonixConfigFile.loadDict()
