...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.resonix", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: ResonixNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [ResonixCapability.canvas.rawValue, ResonixCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(ResonixCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if ResonixLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(ResonixCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            ResonixSystemCommand.which.rawValue,
            ResonixSystemCommand.run.rawValue,
            ResonixSystemCommand.execApprovalsGet.rawValue,
            ResonixSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(ResonixCapability.camera.rawValue) {
        ...
        if capsSet.contains(ResonixCapability.location.rawValue) {
