...
enum LogLocator {
    private static var logDir: URL {
        ...
        return URL(fileURLWithPath: "/tmp/resonix")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("resonix-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("resonix-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/resonix/ (rolling or stdout), or nil if none exist.
