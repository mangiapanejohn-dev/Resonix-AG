...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ResonixProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.resonix", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ResonixKit.AnyCodable]()) {
                $0[$1.key] = ResonixKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ResonixProtocol.AnyCodable?) -> [String: ResonixProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ResonixProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ResonixKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ResonixProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("resonix.control.heartbeat")
    static let controlAgentEvent = Notification.Name("resonix.control.agent")
}
