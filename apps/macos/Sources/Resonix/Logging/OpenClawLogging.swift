...
enum ResonixLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = ResonixOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ResonixFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ResonixLogging.bootstrapIfNeeded()
        let label = ResonixLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ResonixOSLogHandler: LogHandler {
...
struct ResonixFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ResonixLogging.parseLabel(self.label)
