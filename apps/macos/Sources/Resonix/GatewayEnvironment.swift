...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.resonix", category: "gateway.env")
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = ResonixConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.resonixExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "resonix CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "resonix@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "resonix@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "resonix@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "resonix@\(target)"])
            }

        statusHandler("Installing resonix@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed resonix@\(target)")
        } else {
