...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.resonix", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ResonixSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ResonixSessionPreviewEntry,
    ...
    private static func cache(payload: ResonixSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ResonixChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ResonixKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ResonixKit.AnyCodable]) -> [ResonixChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ResonixChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ResonixChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ResonixChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ResonixChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ResonixChatMessage) -> String? {
