...
enum DebugActions {
    private static let verboseDefaultsKey = "resonix.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "resonix.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ResonixPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Resonix", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ResonixPaths.configURL
        guard
