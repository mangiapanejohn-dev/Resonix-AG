...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.resonix", category: "gateway-endpoint")
    ...
    private let logger = Logger(subsystem: "ai.resonix", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "resonix.onboardingSeen")
            initialMode = seen ? .local : .unconfigured
        }

        let port = deps.localPort()
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: ResonixConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: ResonixConfigFile.loadDict())
