...
struct ResonixMacCLI {
    static func main() async {
        ...
        default:
            fputs("resonix-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      resonix-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      resonix-mac discover [--timeout <ms>] [--json] [--include-local]
      resonix-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      resonix-mac connect
      resonix-mac connect --url ws://127.0.0.1:18789 --json
      resonix-mac discover --timeout 3000 --json
      resonix-mac wizard --mode local
    """)
}
