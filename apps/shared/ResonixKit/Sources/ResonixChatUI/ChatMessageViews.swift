...
struct ChatMessageBubble: View {
    let message: ResonixChatMessage
    let style: ResonixChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ResonixChatTheme.userText : ResonixChatTheme.assistantText
    ...
    private var inlineAttachments: [ResonixChatMessageContent] {
    ...
    private var toolCalls: [ResonixChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ResonixChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ResonixChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ResonixChatTheme.onboardingAssistantBubble
        }
        return ResonixChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ResonixChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ResonixChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ResonixChatTheme.userText : ResonixChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ResonixChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ResonixChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ResonixChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ResonixChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ResonixChatTheme.assistantText)
            }
        }
    }
}
