...
private let chatUILogger = Logger(subsystem: "ai.resonix", category: "ResonixChatUI")

@MainActor
@Observable
public final class ResonixChatViewModel {
    public private(set) var messages: [ResonixChatMessage] = []
    ...
    public var attachments: [ResonixPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ResonixChatPendingToolCall] = []
    public private(set) var sessions: [ResonixChatSessionEntry] = []
    private let transport: any ResonixChatTransport
    ...
    private var pendingToolCallsById: [String: ResonixChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ResonixChatTransport) {
    ...
    public var sessionChoices: [ResonixChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [ResonixChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ResonixPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ResonixChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ResonixChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func messageIdentityKey(for message: ResonixChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [ResonixChatMessage],
        incoming: [ResonixChatMessage]) -> [ResonixChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [ResonixChatMessage]) -> [ResonixChatMessage] {
        var result: [ResonixChatMessage] = []
    ...
    private static func dedupeKey(for message: ResonixChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ResonixChatMessageContent] = [
            ResonixChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ResonixChatAttachmentPayload in
            ResonixChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ResonixChatSessionEntry {
        ResonixChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ResonixChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ResonixChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ResonixAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ResonixChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ResonixPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
