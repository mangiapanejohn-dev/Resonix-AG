...
@MainActor
struct ResonixChatComposer: View {
    @Bindable var viewModel: ResonixChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ResonixChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ResonixChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ResonixPendingAttachment.id)
                { (att: ResonixPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ResonixPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ResonixChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ResonixChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ResonixChatTheme.subtleCard)
        .clipShape(Capsule())
    }
