...
private final class TestChatTransport: @unchecked Sendable, ResonixChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ResonixChatHistoryPayload]
    private let sessionsResponses: [ResonixChatSessionsListResponse]

    private let stream: AsyncStream<ResonixChatTransportEvent>
    private let continuation: AsyncStream<ResonixChatTransportEvent>.Continuation

    init(
        historyResponses: [ResonixChatHistoryPayload],
        sessionsResponses: [ResonixChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ResonixChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ResonixChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ResonixChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [ResonixChatAttachmentPayload]) async throws -> ResonixChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return ResonixChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ResonixChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ResonixChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ResonixChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ResonixChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ResonixChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ResonixChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ResonixChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ResonixChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ResonixChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ResonixChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
