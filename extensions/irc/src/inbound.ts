...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ResonixConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ResonixConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ResonixConfig);
