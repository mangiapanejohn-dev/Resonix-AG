...
function resolveGatewayPort(cfg: ResonixPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.RESONIX_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.RESONIXDBOT_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: ResonixPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([
      process.env.RESONIX_GATEWAY_TOKEN,
      process.env.RESONIXDBOT_GATEWAY_TOKEN,
      cfg.gateway?.auth?.token,
    ]) ?? undefined;
  const password =
    pickFirstDefined([
      process.env.RESONIX_GATEWAY_PASSWORD,
      process.env.RESONIXDBOT_GATEWAY_PASSWORD,
...
async function resolveGatewayUrl(api: ResonixPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Resonix iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Resonix iOS app:",
            "",
            "```",
            qrAscii,
            "```",
