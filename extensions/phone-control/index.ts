...
import type { ResonixPluginApi, ResonixPluginService } from "resonix/plugin-sdk";
...
function normalizeDenyList(cfg: ResonixPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: ResonixPluginApi["config"]): string[] {
...
export default function register(api: ResonixPluginApi) {
  ...
  const timerService: ResonixPluginService = {
