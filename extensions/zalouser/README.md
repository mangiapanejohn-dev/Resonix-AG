...
Resonix extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
resonix channels login --channel zalouser
...
```bash
resonix message send --channel zalouser --target <threadId> --message "Hello from Resonix!"
```
...
```bash
resonix channels login --channel zalouser              # Login via QR
resonix channels login --channel zalouser --account work
resonix channels status --probe
resonix channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
resonix directory self --channel zalouser
resonix directory peers list --channel zalouser --query "name"
resonix directory groups list --channel zalouser --query "work"
resonix directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
resonix message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
resonix message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `resonix channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
resonix channels login --channel zalouser --account work
resonix message send --channel zalouser --account work --target <id> --message "Hello"
