...
`resonix.invoke --each`) without adding new Resonix code per workflow.
...
This extension depends on Resonix internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Resonix extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.resonix/extensions` as a standalone plugin directory.
