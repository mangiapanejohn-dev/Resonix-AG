...
metadata: { "resonix": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Resonix `sessions_spawn`
- **File I/O** == Resonix `read`/`write`
- **Remote fetch** == Resonix `web_fetch` (or `exec` with curl when POST is required)
