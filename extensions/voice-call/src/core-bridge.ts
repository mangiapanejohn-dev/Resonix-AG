...
function resolveResonixRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.RESONIX_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set RESONIX_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveResonixRoot(), "dist", "extensionAPI.js");
