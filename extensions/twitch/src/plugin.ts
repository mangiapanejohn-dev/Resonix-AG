/**
 * Twitch channel plugin for Resonix.
 *
...
import { buildChannelConfigSchema } from "resonix/plugin-sdk";
 ...
 * for Resonix. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ResonixConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ResonixConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ResonixConfig): boolean => {
