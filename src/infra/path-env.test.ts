...
let ensureResonixCliOnPath: typeof import("./path-env.js").ensureResonixCliOnPath;

describe("ensureResonixCliOnPath", () => {
  const envKeys = [
    "PATH",
    "RESONIX_PATH_BOOTSTRAPPED",
    "RESONIX_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureResonixCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling resonix exists", () => {
    const tmp = abs("/tmp/resonix-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "resonix");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.RESONIX_PATH_BOOTSTRAPPED;

    ensureResonixCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.RESONIX_PATH_BOOTSTRAPPED = "1";
    ensureResonixCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/resonix-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "resonix");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/resonix-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "resonix");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "resonix");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/resonix-path/case-linuxbrew");
