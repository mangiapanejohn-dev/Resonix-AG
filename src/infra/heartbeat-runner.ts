...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ResonixConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ResonixConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ResonixConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: ResonixConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: ResonixConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ResonixConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ResonixConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ResonixConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
