...
vi.mock("./resonix-root.js", () => ({
  resolveResonixPackageRoot: vi.fn(async () => null),
  resolveResonixPackageRootSync: vi.fn(() => null),
}));
...
let resonixRoot: typeof import("./resonix-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    resonixRoot = await import("./resonix-root.js");
  });
  ...
  it("uses resolveResonixPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/resonix");
    (
      resonixRoot.resolveResonixPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/resonix"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "resonix" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "resonix.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-resonix");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/resonix-bundle");
    (
      resonixRoot.resolveResonixPackageRootSync as unknown as ReturnType<typeof vi.fn>
