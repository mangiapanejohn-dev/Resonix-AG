...
function candidateBinDirs(opts: EnsureResonixPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `resonix` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "resonix");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.RESONIX_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "resonix"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `resonix` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureResonixCliOnPath(opts: EnsureResonixPathOpts = {}) {
  if (isTruthyEnvValue(process.env.RESONIX_PATH_BOOTSTRAPPED)) {
    return;
  }
