...
import { resolveResonixPackageRoot, resolveResonixPackageRootSync } from "./resonix-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveResonixPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "resonix" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveResonixPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
