...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.RESONIX_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Resonix";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Resonix\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.RESONIX_MDNS_HOSTNAME?.trim() ||
    process.env.RESONIXDBOT_MDNS_HOSTNAME?.trim() ||
    "resonix";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
