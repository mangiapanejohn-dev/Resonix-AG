...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.RESONIX_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.RESONIX_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set RESONIX_APNS_TEAM_ID and RESONIX_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.RESONIX_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.RESONIX_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.RESONIX_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set RESONIX_APNS_PRIVATE_KEY_P8 or RESONIX_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading RESONIX_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createResonixPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    resonix: createResonixPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
