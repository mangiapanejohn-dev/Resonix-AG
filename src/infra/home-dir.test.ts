...
describe("resolveEffectiveHomeDir", () => {
  it("prefers RESONIX_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/resonix-home"));
  });
  ...
  it("expands RESONIX_HOME when set to ~", () => {
    const env = {
      RESONIX_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for RESONIX_HOME", () => {
    const result = resolveRequiredHomeDir(
      { RESONIX_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when RESONIX_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ RESONIX_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { RESONIX_HOME: "/srv/resonix-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/resonix-home")}/x`);
  });
