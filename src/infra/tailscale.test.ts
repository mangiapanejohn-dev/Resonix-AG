...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.RESONIX_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.RESONIX_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.RESONIX_TEST_TAILSCALE_BINARY;
    } else {
      process.env.RESONIX_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
