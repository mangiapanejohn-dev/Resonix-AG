...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ResonixConfig | null = null;
  ...
  get(key: string, cfg: ResonixConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ResonixConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: ResonixConfig): void {
