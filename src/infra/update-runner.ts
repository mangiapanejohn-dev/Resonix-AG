...
const DEFAULT_PACKAGE_NAME = "resonix";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "resonix-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "resonix.mjs");
    ...
    const doctorStep = await runStep(
      step("resonix doctor", doctorArgv, gitRoot, { RESONIX_UPDATE_IN_PROGRESS: "1" }),
    );
