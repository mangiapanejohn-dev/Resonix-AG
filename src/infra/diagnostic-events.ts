...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __resonixDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__resonixDiagnosticEventsState) {
    globalStore.__resonixDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__resonixDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: ResonixConfig): boolean {
