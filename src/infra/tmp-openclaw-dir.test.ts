...
import { POSIX_RESONIX_TMP_DIR, resolvePreferredResonixTmpDir } from "./tmp-resonix-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredResonixTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `resonix-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredResonixTmpDir({
    accessSync,
...
describe("resolvePreferredResonixTmpDir", () => {
  it("prefers /tmp/resonix when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_RESONIX_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/resonix when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_RESONIX_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/resonix when /tmp/resonix is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/resonix when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/resonix is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/resonix is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/resonix is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
