...
export function triggerResonixRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.RESONIX_SYSTEMD_UNIT,
        process.env.RESONIX_PROFILE,
      );
  ...
  const label =
    process.env.RESONIX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.RESONIX_PROFILE);
