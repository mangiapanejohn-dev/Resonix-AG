...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "resonix-exec-approvals-"));
}
...
describe("resolve exec approvals defaults", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    const prevResonixHome = process.env.RESONIX_HOME;
    try {
      process.env.RESONIX_HOME = dir;
      expect(path.normalize(resolveExecApprovalsPath())).toBe(
        path.normalize(path.join(dir, ".resonix", "exec-approvals.json")),
      );
      expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
        path.normalize(path.join(dir, ".resonix", "exec-approvals.sock")),
      );
    } finally {
      if (prevResonixHome === undefined) {
        delete process.env.RESONIX_HOME;
      } else {
        process.env.RESONIX_HOME = prevResonixHome;
      }
    }
  });
});
...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    try {
      process.env.RESONIX_HOME = dir;
      const approvalsPath = path.join(dir, ".resonix", "exec-approvals.json");
