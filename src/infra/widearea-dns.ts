...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.RESONIX_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*resonix-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "resonix";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "resonix");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "resonix-gw");
  ...
  records.push(`_resonix-gw._tcp IN PTR ${instanceLabel}._resonix-gw._tcp`);
  records.push(`${instanceLabel}._resonix-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._resonix-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; resonix-content-hash: ${contentHash}\n${contentBody}`;
}
