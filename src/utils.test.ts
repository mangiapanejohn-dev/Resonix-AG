...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "resonix-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "resonix-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "resonix-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "resonix-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.resonix when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "resonix-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers RESONIX_HOME over HOME", () => {
    vi.stubEnv("RESONIX_HOME", "/srv/resonix-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/resonix-home"));
...
describe("shortenHomePath", () => {
  it("uses $RESONIX_HOME prefix when RESONIX_HOME is set", () => {
    vi.stubEnv("RESONIX_HOME", "/srv/resonix-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/resonix-home")}/.resonix/resonix.json`)).toBe(
      "$RESONIX_HOME/.resonix/resonix.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $RESONIX_HOME replacement when RESONIX_HOME is set", () => {
    vi.stubEnv("RESONIX_HOME", "/srv/resonix-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/resonix-home")}/.resonix/resonix.json`),
    ).toBe("config: $RESONIX_HOME/.resonix/resonix.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/resonix")).toBe(path.resolve(os.homedir(), "resonix"));
  });
  ...
  it("prefers RESONIX_HOME for tilde expansion", () => {
    vi.stubEnv("RESONIX_HOME", "/srv/resonix-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/resonix")).toBe(path.resolve("/srv/resonix-home", "resonix"));
