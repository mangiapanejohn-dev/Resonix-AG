...
function summarizeGroupPolicy(cfg: ResonixConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: ResonixConfig): boolean {
...
function collectModels(cfg: ResonixConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: ResonixConfig): Set<string> {
  const baseCfg: ResonixConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: ResonixConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ResonixConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ResonixConfig): boolean {
...
function isBrowserEnabled(cfg: ResonixConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: ResonixConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: ResonixConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep RESONIX_STATE_DIR on a local-only volume and re-run "${formatCliCommand("resonix security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: ResonixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer RESONIX_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const resonixGatewayToken =
    typeof env.RESONIX_GATEWAY_TOKEN === "string" && env.RESONIX_GATEWAY_TOKEN.trim()
      ? env.RESONIX_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-resonix-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: ResonixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: ResonixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: ResonixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: ResonixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: ResonixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: ResonixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
