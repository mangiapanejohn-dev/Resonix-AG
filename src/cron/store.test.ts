...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "resonix-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses RESONIX_HOME for tilde expansion", () => {
    vi.stubEnv("RESONIX_HOME", "/srv/resonix-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/resonix-home", "cron", "jobs.json"));
  });
});
