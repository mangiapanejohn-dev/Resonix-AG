...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.RESONIX_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service RESONIX_GATEWAY_TOKEN does not match gateway.auth.token in resonix.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `resonix gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
