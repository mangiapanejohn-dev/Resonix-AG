...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.RESONIX_GATEWAY_PORT).toBe("18789");
    expect(env.RESONIX_GATEWAY_TOKEN).toBe("secret");
    expect(env.RESONIX_SERVICE_MARKER).toBe("resonix");
    expect(env.RESONIX_SERVICE_KIND).toBe("gateway");
    expect(typeof env.RESONIX_SERVICE_VERSION).toBe("string");
    expect(env.RESONIX_SYSTEMD_UNIT).toBe("resonix-gateway.service");
    if (process.platform === "darwin") {
      expect(env.RESONIX_LAUNCHD_LABEL).toBe("ai.resonix.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", RESONIX_PROFILE: "work" },
      port: 18789,
    });
    expect(env.RESONIX_SYSTEMD_UNIT).toBe("resonix-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.RESONIX_LAUNCHD_LABEL).toBe("ai.resonix.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".resonix"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", RESONIX_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".resonix-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", RESONIX_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".resonix"));
  });

  it("uses RESONIX_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", RESONIX_STATE_DIR: "/var/lib/resonix" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/resonix"));
  });

  it("expands ~ in RESONIX_STATE_DIR", () => {
    const env = { HOME: "/Users/test", RESONIX_STATE_DIR: "~/resonix-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/resonix-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { RESONIX_STATE_DIR: "C:\\State\\resonix" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\resonix");
  });
});
