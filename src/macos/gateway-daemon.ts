...
declare const __RESONIX_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __RESONIX_VERSION__ === "string" && __RESONIX_VERSION__) ||
  process.env.RESONIX_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `resonix --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.RESONIX_GATEWAY_PORT ??
    process.env.RESONIXDBOT_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.RESONIX_GATEWAY_BIND ??
    process.env.RESONIXDBOT_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
    bindRaw === "loopback" ||
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (RESONIX_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
