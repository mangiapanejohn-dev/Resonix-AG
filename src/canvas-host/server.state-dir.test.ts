...
describe("canvas host state dir defaults", () => {
  ...
  it("uses RESONIX_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "resonix-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Resonix Canvas");
    } finally {
      await handler.close();
