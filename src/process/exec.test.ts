...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["RESONIX_BASE_ENV"]);
    process.env.RESONIX_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.RESONIX_BASE_ENV ?? "") + "|" + (process.env.RESONIX_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { RESONIX_TEST_ENV: "ok" },
        },
      );
