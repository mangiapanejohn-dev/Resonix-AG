...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.RESONIX_STATE_DIR;
  previousConfigPath = process.env.RESONIX_CONFIG_PATH;
  previousSkipBrowserControl = process.env.RESONIX_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.RESONIX_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.RESONIX_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.RESONIX_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.RESONIX_SKIP_CHANNELS;
  previousSkipProviders = process.env.RESONIX_SKIP_PROVIDERS;
  previousSkipCron = process.env.RESONIX_SKIP_CRON;
  previousMinimalGateway = process.env.RESONIX_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "resonix-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.RESONIX_STATE_DIR = path.join(tempHome, ".resonix");
  delete process.env.RESONIX_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.RESONIX_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.RESONIX_SKIP_GMAIL_WATCHER = "1";
  process.env.RESONIX_SKIP_CANVAS_HOST = "1";
  process.env.RESONIX_SKIP_CHANNELS = "1";
  process.env.RESONIX_SKIP_PROVIDERS = "1";
  process.env.RESONIX_SKIP_CRON = "1";
  process.env.RESONIX_TEST_MINIMAL_GATEWAY = "1";
  process.env.RESONIX_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "resonix-test-no-bundled-extensions")
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "resonix-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".resonix-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.RESONIX_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.RESONIX_CONFIG_PATH;
    } else {
      process.env.RESONIX_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.RESONIX_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.RESONIX_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.RESONIX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.RESONIX_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.RESONIX_SKIP_CANVAS_HOST;
    } else {
      process.env.RESONIX_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.RESONIX_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.RESONIX_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.RESONIX_SKIP_CHANNELS;
    } else {
      process.env.RESONIX_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.RESONIX_SKIP_PROVIDERS;
    } else {
      process.env.RESONIX_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.RESONIX_SKIP_CRON;
    } else {
      process.env.RESONIX_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.RESONIX_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.RESONIX_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["RESONIX_GATEWAY_TOKEN"]);
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.RESONIX_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.RESONIX_GATEWAY_PASSWORD;
