...
const { createResonixTools } = await import("../agents/resonix-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["RESONIX_GATEWAY_PORT", "RESONIX_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.RESONIX_GATEWAY_PORT = String(gatewayPort);
  process.env.RESONIX_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createResonixTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("RESONIX_CONFIG_PATH missing in gateway test environment");
    }
