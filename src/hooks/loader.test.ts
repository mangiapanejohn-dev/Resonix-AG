...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "resonix-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.RESONIX_BUNDLED_HOOKS_DIR;
    process.env.RESONIX_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.RESONIX_BUNDLED_HOOKS_DIR;
    } else {
      process.env.RESONIX_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
