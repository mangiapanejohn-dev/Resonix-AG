...
`;
    ...
    expect(parsed.resonix.emoji).toBe("💾");
    expect(parsed.resonix.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.resonix.emoji).toBe("📝");
    expect(parsed.resonix.events).toEqual(["command"]);
    expect(parsed.resonix.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.resonix.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"resonix": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"resonix": {"events": ["test"]}}');
  });
...
describe("resolveResonixMetadata", () => {
  it("extracts resonix metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveResonixMetadata(frontmatter);
  ...
  it("returns undefined when resonix key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        resonix: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Resonix" },
            { id: "npm", kind: "npm", package: "@resonix/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@resonix/hook");
  });
...
`;
    ...
    const resonix = resolveResonixMetadata(frontmatter);
    expect(resonix).toBeDefined();
    expect(resonix?.emoji).toBe("💾");
    expect(resonix?.events).toEqual(["command:new"]);
    expect(resonix?.requires?.config).toEqual(["workspace.dir"]);
    expect(resonix?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const resonix = resolveResonixMetadata(frontmatter);
    expect(resonix?.emoji).toBe("disk");
    expect(resonix?.events).toEqual(["command:new"]);
  });
});
