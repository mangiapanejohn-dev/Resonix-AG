...
homepage: https://docs.resonix.ai/automation/hooks#command-logger
metadata:
  {
    "resonix":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.resonix/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.resonix/logs/commands.log ~/.resonix/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/resonix`:
   ```
   /home/username/.resonix/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
