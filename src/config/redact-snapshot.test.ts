...
import type { ConfigFileSnapshot } from "./types.resonix.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.resonix/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/resonix/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/resonix/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/resonix/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/resonix/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = ResonixSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "ResonixConfig";
    const hints = mapSensitivePaths(ResonixSchema, "", {});
