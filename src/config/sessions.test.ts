...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "resonix-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-resonix",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-resonix#general");
  });
  ...
  it("derives session transcripts dir from RESONIX_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { RESONIX_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.RESONIX_STATE_DIR = "/custom/state";
    try {
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const prev = process.env.RESONIX_STATE_DIR;
    const stateDir = path.resolve("/home/user/.resonix");
  ...
  it("resolves cross-agent paths when RESONIX_STATE_DIR differs from stored paths", () => {
    const prev = process.env.RESONIX_STATE_DIR;
    process.env.RESONIX_STATE_DIR = path.resolve("/different/state");
    try {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    const prev = process.env.RESONIX_STATE_DIR;
    process.env.RESONIX_STATE_DIR = path.resolve("/home/user/.resonix");
    try {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
