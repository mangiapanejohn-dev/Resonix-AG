...
import type { ResonixConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ResonixConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ResonixConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ResonixSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ResonixConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ResonixConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as ResonixConfig,
  };
}
