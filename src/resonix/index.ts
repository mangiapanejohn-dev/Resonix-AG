...
export class ResonixCognitionEngine {
  ...
  private startPeriodicTasks(): void {
    ...
    // 定期迭代策略
    const iterateInterval = setInterval(
      async () => {
        console.log("[Resonix] 迭代学习策略...");
        const result = await this.programMemory.iterateStrategies();
        console.log(
          `[Resonix] 策略迭代: 提升${result.promoted.length}, 降级${result.demoted.length}, 删除${result.removed.length}`,
        );
      },
      7 * 24 * 60 * 60 * 1000,
    ); // 每周
  ...
  async performAutonomousLearning(): Promise<void> {
    try {
      ...
      // 2. 为每个高优先级需求生成学习路径
      for (const demand of demands.slice(0, 3)) {
        // 最多同时处理3个
        // 创建学习进度
  ...
  async processUserFeedback(feedback: string, relatedKnowledgeId?: string): Promise<void> {
    ...
    await this.episodicMemory.log({
      ...
      sentiment: /错误|不对|差/i.test(feedback)
        ? "negative"
        : /好|棒|赞/i.test(feedback)
          ? "positive"
          : "neutral",
    });

    // 2. 检测偏差
    const deviation = await this.deviationCorrection.detectFromUserFeedback(
      feedback,
      relatedKnowledgeId,
    );
    ...
    if (relatedKnowledgeId) {
      const delta = /错误|不对/i.test(feedback) ? -1 : /好|棒/i.test(feedback) ? 1 : 0;
      if (delta !== 0) {
