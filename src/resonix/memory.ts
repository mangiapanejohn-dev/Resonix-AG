...
// Memory directories
const MEMORY_BASE = path.join(CONFIG_DIR, "resonix-memory");
...
export class EnhancedMemorySystem {
   ...
   * Get fact memories
   */
  async getFacts(query: string, limit = 5): Promise<MemoryEntry[]> {
    return this.search(query, { type: "fact", limit, minImportance: 0.3 });
  }

  /**
   * Get preference memories
   */
  async getPreferences(query: string, limit = 5): Promise<MemoryEntry[]> {
    return this.search(query, { type: "preference", limit, minImportance: 0.2 });
  }

  /**
   * Get relation memories
   */
  async getRelations(query: string, limit = 5): Promise<MemoryEntry[]> {
    return this.search(query, { type: "relation", limit, minImportance: 0.3 });
  }

  /**
   * Get skill memories
   */
  async getSkills(query: string, limit = 5): Promise<MemoryEntry[]> {
    return this.search(query, { type: "skill", limit, minImportance: 0.4 });
  }

  /**
   * Get goal memories
   */
  async getGoals(query: string, limit = 5): Promise<MemoryEntry[]> {
    return this.search(query, { type: "goal", limit, minImportance: 0.3 });
  }

  /**
   * Get thought memories
   */
  async getThoughts(query: string, limit = 5): Promise<MemoryEntry[]> {
    return this.search(query, { type: "thought", limit, minImportance: 0.2 });
  }
