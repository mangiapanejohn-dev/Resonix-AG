...
const GOOGLE_LIVE_SINGLE_KEY = "RESONIX_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "RESONIX_LIVE_ANTHROPIC_KEY",
    listVar: "RESONIX_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "RESONIX_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `RESONIX_LIVE_${base}_KEY`;
