...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  const envSnapshot = captureEnv(["RESONIX_STATE_DIR", "RESONIX_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tmpDir: string;
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveResonixAgentDir() returns mainAgentDir
    process.env.RESONIX_STATE_DIR = tmpDir;
    process.env.RESONIX_AGENT_DIR = mainAgentDir;
