...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.RESONIX_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".resonix", `workspace-${profile}`);
  }
  return path.join(home, ".resonix", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".resonix";
