...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "resonix.sandbox=1");
  args.push("--label", `resonix.sessionKey=${params.scopeKey}`);
  args.push("--label", `resonix.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `resonix.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "resonix.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`resonix sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`resonix sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("resonix sandbox recreate --all");
}
