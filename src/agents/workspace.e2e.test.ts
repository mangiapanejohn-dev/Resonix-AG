...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses RESONIX_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      RESONIX_HOME: "/srv/resonix-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/resonix-home"), ".resonix", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".resonix", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("resonix-workspace-");
