...
function resolveDefaultModelRef(cfg?: ResonixConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: ResonixConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureResonixModelsJson(effectiveCfg, params.agentDir);
