...
type WebSearchConfig = NonNullable<ResonixConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ResonixConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("resonix configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.resonix.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://resonix.ai",
      "X-Title": "Resonix Web Search",
    },
