...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["RESONIX_STATE_DIR"]);
  let tempStateDir: string | null = null;

  const writePersistedRegistry = async (persisted: Record<string, unknown>) => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "resonix-subagent-"));
  ...
  it("uses isolated temp state when RESONIX_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "resonix-test-state"));
  });
});
