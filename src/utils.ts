...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".resonix");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.RESONIX_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$RESONIX_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via RESONIX_STATE_DIR.
