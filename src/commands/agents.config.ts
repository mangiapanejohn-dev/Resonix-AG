...
function resolveAgentName(cfg: ResonixConfig, agentId: string) {
...
function resolveAgentModel(cfg: ResonixConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ResonixConfig): AgentSummary[] {
