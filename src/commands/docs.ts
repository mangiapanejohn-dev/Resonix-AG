...
const SEARCH_TOOL = "https://docs.resonix.ai/mcp.SearchResonix";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.resonix.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('resonix docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.resonix.ai/");
      runtime.log(`Search: ${formatCliCommand('resonix docs "your query"')}`);
    }
    return;
  }
