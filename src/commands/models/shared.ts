...
export async function loadValidConfigOrThrow(): Promise<ResonixConfig> {
...
export async function updateConfig(
  mutator: (cfg: ResonixConfig) => ResonixConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ResonixConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ResonixConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("resonix agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
