...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveResonixAgentDir: vi.fn().mockReturnValue("/tmp/resonix-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/resonix-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/resonix-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveResonixAgentDir: mocks.resolveResonixAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/resonix-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveResonixAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/resonix-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/resonix-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/resonix-agent-custom");
