...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveResonixAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("resonix models auth setup-token")}\` or \`${formatCliCommand("resonix configure")}\`.`
          : `Run \`${formatCliCommand("resonix configure")}\` or set an API key env var.`;
