...
export const resolveResonixPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadResonixPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveResonixPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.RESONIX_STATE_DIR;
  originalUpdateInProgress = process.env.RESONIX_UPDATE_IN_PROGRESS;
  process.env.RESONIX_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "resonix-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.RESONIX_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.RESONIX_UPDATE_IN_PROGRESS;
  } else {
    process.env.RESONIX_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
