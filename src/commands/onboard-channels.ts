...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("resonix pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`resonix pairing approve ${policy.channel} <code>`)}`,
...
export async function setupChannels(
  ...
  const filteredPrimer = corePrimer.filter((channel) =>
    ENABLED_CHANNELS.has(channel.id as ChannelChoice),
  );
  ...
  const primerChannels = [
    ...
    ...installedPlugins
      .filter(
        (plugin) => !coreIds.has(plugin.id) && ENABLED_CHANNELS.has(plugin.id as ChannelChoice),
      )
      .map((plugin) => ({
        id: plugin.id,
    ...
    ...catalogEntries
      .filter(
        (entry) =>
          !coreIds.has(entry.id as ChannelChoice) &&
          ENABLED_CHANNELS.has(entry.id as ChannelChoice),
      )
      .map((entry) => ({
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("resonix channels add")}\``,
        },
      ],
