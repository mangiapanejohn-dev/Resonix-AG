...
beforeAll(() => {
  envSnapshot = captureEnv(["RESONIX_PROFILE"]);
  process.env.RESONIX_PROFILE = "isolated";
});
...
vi.mock("../infra/resonix-root.js", () => ({
  resolveResonixPackageRoot: vi.fn().mockResolvedValue("/tmp/resonix"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/resonix/pnpm-lock.yaml",
      markerPath: "/tmp/resonix/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Resonix status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("resonix status --all") ||
          l.includes("resonix --profile isolated status --all") ||
          l.includes("resonix status --all") ||
          l.includes("resonix --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.RESONIX_GATEWAY_TOKEN = "abcd1234";
    try {
