...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "RESONIX_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("RESONIX_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv RESONIX_GATEWAY_TOKEN");
    expect(message).not.toContain("RESONIX_GATEWAY_PASSWORD");
  });
