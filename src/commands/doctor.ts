...
function resolveMode(cfg: ResonixConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Resonix doctor");

  const root = await resolveResonixPackageRoot({
  ...
  let cfg: ResonixConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("resonix configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("resonix config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("resonix setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("resonix doctor --fix")}" to apply changes.`);
  }
