...
function listAgents(cfg: ResonixConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: ResonixConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ResonixConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<ResonixConfig, EvaluatedBindingsCache>();
