...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.RESONIX_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.RESONIX_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<ResonixConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live Resonix docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  },
  {
    ...
    register: async (program) => {
      ...
      mod.registerResonixbotCli(program);
    },
  },
  {
    name: "pairing",
  ...
  {
    name: "plugins",
    description: "Manage Resonix plugins and extensions",
    hasSubcommands: true,
    register: async (program) => {
