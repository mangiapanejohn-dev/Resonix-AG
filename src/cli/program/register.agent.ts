...
${formatHelpExamples([
  ['resonix agent --to +15555550123 --message "status update"', "Start a new session."],
  ['resonix agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'resonix agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'resonix agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['resonix agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'resonix agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.resonix.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.resonix.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['resonix agents set-identity --agent main --name "Resonix" --emoji "👾"', "Set name + emoji."],
  ["resonix agents set-identity --agent main --avatar avatars/resonix.png", "Set avatar path."],
  [
    "resonix agents set-identity --workspace ~/.resonix/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "resonix agents set-identity --identity-file ~/.resonix/workspace/IDENTITY.md --agent main",
