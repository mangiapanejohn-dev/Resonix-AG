...
const EXAMPLES = [
  ...
  ["resonix gateway --port 18789", "Run the WebSocket Gateway locally."],
  ["resonix --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["resonix gateway --force", "Kill anything bound to the default gateway port, then start it."],
  ["resonix gateway ...", "Gateway control via WebSocket."],
  [
    'resonix agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'resonix message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.resonix-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates RESONIX_STATE_DIR/RESONIX_CONFIG_PATH under ~/.resonix-<name>)",
    );
