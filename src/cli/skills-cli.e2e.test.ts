...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["RESONIX_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "resonix-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "resonix-bundled-skills-test-"));
    process.env.RESONIX_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
