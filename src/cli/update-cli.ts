...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Resonix and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["resonix update", "Update a source checkout (git)"],
        ["resonix update --channel beta", "Switch to beta channel (git + npm)"],
        ["resonix update --channel dev", "Switch to dev channel (git + npm)"],
        ["resonix update --tag beta", "One-off update to a dist-tag or version"],
        ["resonix update --no-restart", "Update without restarting the service"],
        ["resonix update --json", "Output result as JSON"],
        ["resonix update --yes", "Non-interactive (accept downgrade prompts)"],
        ["resonix update wizard", "Interactive update wizard"],
        ["resonix --update", "Shorthand for resonix update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run resonix update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.resonix.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["resonix update status", "Show channel + version status."],
          ["resonix update status --json", "JSON output."],
          ["resonix update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.resonix.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
