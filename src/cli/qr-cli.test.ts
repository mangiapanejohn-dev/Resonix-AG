...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("RESONIX_GATEWAY_TOKEN", "");
    vi.stubEnv("RESONIXDBOT_GATEWAY_TOKEN", "");
    vi.stubEnv("RESONIX_GATEWAY_PASSWORD", "");
    vi.stubEnv("RESONIXDBOT_GATEWAY_PASSWORD", "");
  });
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("resonix devices approve <requestId>");
  });
