...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("resonix node install"), formatCliCommand("resonix node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("resonix node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    RESONIX_LOG_PREFIX: baseEnv.RESONIX_LOG_PREFIX ?? "node",
