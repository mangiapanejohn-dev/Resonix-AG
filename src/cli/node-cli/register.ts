    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "resonix node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["resonix node status", "Check node host service status."],
          ["resonix node install", "Install the node host service."],
          ["resonix node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.resonix.ai/cli/node")}\n`,
    );
