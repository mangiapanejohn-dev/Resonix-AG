...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("resonix skills list")}\` to see all skills.`
      : "No skills found.";
  ...
  return appendResonixHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendResonixHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("resonix skills list")}\` to see available skills.`,
      opts.json,
    );
  }
