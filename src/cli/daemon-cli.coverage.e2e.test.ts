...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    RESONIX_STATE_DIR: process.env.RESONIX_STATE_DIR,
    RESONIX_CONFIG_PATH: process.env.RESONIX_CONFIG_PATH,
    RESONIX_GATEWAY_PORT: process.env.RESONIX_GATEWAY_PORT,
    RESONIX_PROFILE: process.env.RESONIX_PROFILE,
  };

  beforeEach(() => {
    process.env.RESONIX_STATE_DIR = "/tmp/resonix-cli-state";
    process.env.RESONIX_CONFIG_PATH = "/tmp/resonix-cli-state/resonix.json";
    delete process.env.RESONIX_GATEWAY_PORT;
    delete process.env.RESONIX_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.RESONIX_STATE_DIR !== undefined) {
      process.env.RESONIX_STATE_DIR = originalEnv.RESONIX_STATE_DIR;
    } else {
      delete process.env.RESONIX_STATE_DIR;
    }

    if (originalEnv.RESONIX_CONFIG_PATH !== undefined) {
      process.env.RESONIX_CONFIG_PATH = originalEnv.RESONIX_CONFIG_PATH;
    } else {
      delete process.env.RESONIX_CONFIG_PATH;
    }

    if (originalEnv.RESONIX_GATEWAY_PORT !== undefined) {
      process.env.RESONIX_GATEWAY_PORT = originalEnv.RESONIX_GATEWAY_PORT;
    } else {
      delete process.env.RESONIX_GATEWAY_PORT;
    }

    if (originalEnv.RESONIX_PROFILE !== undefined) {
      process.env.RESONIX_PROFILE = originalEnv.RESONIX_PROFILE;
    } else {
      delete process.env.RESONIX_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        RESONIX_PROFILE: "dev",
        RESONIX_STATE_DIR: "/tmp/resonix-daemon-state",
        RESONIX_CONFIG_PATH: "/tmp/resonix-daemon-state/resonix.json",
        RESONIX_GATEWAY_PORT: "19001",
      },
