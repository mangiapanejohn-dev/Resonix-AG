    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["resonix nodes status", "List known nodes with live status."],
          ["resonix nodes pairing pending", "Show pending node pairing requests."],
          ['resonix nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["resonix nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.resonix.ai/cli/nodes")}\n`,
    );
