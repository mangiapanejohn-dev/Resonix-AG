...
import { discoverResonixPlugins } from "../../plugins/discovery.js";
import type { ResonixPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, ResonixPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["RESONIX_PLUGIN_CATALOG_PATHS", "RESONIX_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<ResonixPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: ResonixPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: ResonixPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverResonixPlugins({ workspaceDir: options.workspaceDir });
