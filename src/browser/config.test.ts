...
describe("browser config", () => {
  ...
  it("derives default ports from RESONIX_GATEWAY_PORT when unset", () => {
    const prev = process.env.RESONIX_GATEWAY_PORT;
    process.env.RESONIX_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      expect(resonix?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.RESONIX_GATEWAY_PORT;
      } else {
        process.env.RESONIX_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      expect(resonix?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.RESONIX_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "resonix");
