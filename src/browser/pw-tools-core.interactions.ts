  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-resonix-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-resonix-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-resonix-labels", "1");
          ...
          tag.setAttribute("data-resonix-labels", "1");
