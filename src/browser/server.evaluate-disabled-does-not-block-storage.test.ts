  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          resonix: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.RESONIX_GATEWAY_PORT;
    process.env.RESONIX_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.RESONIX_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.RESONIX_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayToken === undefined) {
      ...
      process.env.RESONIX_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.RESONIX_GATEWAY_PASSWORD;
    } else {
      process.env.RESONIX_GATEWAY_PASSWORD = prevGatewayPassword;
    }
