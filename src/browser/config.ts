import type { BrowserConfig, BrowserProfileConfig, ResonixConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_RESONIX_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_RESONIX_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "resonix" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_RESONIX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_RESONIX_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_RESONIX_BROWSER_ENABLED;
