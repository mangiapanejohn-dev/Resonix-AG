...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredResonixTmpDir: vi.fn(() => "/tmp/resonix"),
}));
vi.mock("../infra/tmp-resonix-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredResonixTmpDir.mockReturnValue("/tmp/resonix");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredResonixTmpDir.mockReturnValue("/tmp/resonix-preferred");
    const { res, outPath } = await waitForImplicitDownloadOutput({
      downloadUrl: "https://example.com/file.bin",
      suggestedFilename: "file.bin",
    });
    expect(typeof outPath).toBe("string");
    const expectedRootedDownloadsDir = path.join(path.sep, "tmp", "resonix-preferred", "downloads");
    const expectedDownloadsTail = `${path.join("tmp", "resonix-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredResonixTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "resonix-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "resonix-preferred", "downloads")}${path.sep}`),
    );
  });
