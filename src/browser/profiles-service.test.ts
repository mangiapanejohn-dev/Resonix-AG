...
vi.mock("./chrome.js", () => ({
  resolveResonixUserDataDir: vi.fn(() => "/tmp/resonix-test/resonix/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveResonixUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "resonix-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveResonixUserDataDir).mockReturnValue(userDataDir);
