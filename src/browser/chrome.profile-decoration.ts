...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".resonix-profile-decorated");
}
 ...
 */
export function decorateResonixProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_RESONIX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_RESONIX_BROWSER_COLOR).toUpperCase();
