...
import { loadResonixPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `resonix-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.RESONIX_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "resonix.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.RESONIX_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadResonixPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.RESONIX_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadResonixPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadResonixPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
