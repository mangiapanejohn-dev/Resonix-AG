...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ResonixPluginDefinition;
  register?: ResonixPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ResonixPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ResonixPluginDefinition;
...
export function loadResonixPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverResonixPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ResonixPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as ResonixPluginModule;
    } catch (err) {
