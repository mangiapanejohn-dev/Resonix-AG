...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `resonix-e2e-${name}-`));
  const configDir = path.join(homeDir, ".resonix");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          RESONIX_SKIP_BROWSER_CONTROL_SERVER: "1",
