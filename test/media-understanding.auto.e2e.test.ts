...
import type { ResonixConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("resonix-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("resonix-bin-sherpa-");
      const modelDir = await makeTempDir("resonix-sherpa-model-");
      ...
      const cfg: ResonixConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("resonix-bin-whispercpp-");
      const modelDir = await makeTempDir("resonix-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("resonix-bin-gemini-");
      ...
      const cfg: ResonixConfig = { tools: { media: { image: {} } } };
