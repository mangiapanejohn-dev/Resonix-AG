...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    resonixHome: process.env.RESONIX_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("RESONIX_HOME", snapshot.resonixHome);
  restoreKey("RESONIX_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked RESONIX_HOME.
  delete process.env.RESONIX_HOME;
  process.env.RESONIX_STATE_DIR = path.join(base, ".resonix");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "resonix-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".resonix", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
