...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/resonix/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Resonix Re-Auth" -c "After getting key, run: ssh $SERVER '~/resonix/scripts/mobile-reauth.sh'" --id resonix-auth
        ;;
    *)
