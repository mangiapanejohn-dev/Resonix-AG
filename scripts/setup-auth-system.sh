#!/bin/bash
# Setup Resonix Auth Management System
...
echo "=== Resonix Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'resonix-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Resonix message: Send warning via Resonix itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/resonix-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/resonix-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/resonix-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now resonix-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/ResonixdAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/ResonixdAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Resonixd*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (ResonixdAuth) shows status and opens auth URL if needed."
echo "The full widget (ResonixdAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status resonix-auth-monitor.timer"
echo ""
echo "Quick commands:"
