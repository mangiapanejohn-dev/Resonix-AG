...
# Build and bundle Resonix into a minimal .app we can open.
# Outputs to dist/Resonix.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Resonix.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Resonix"
BUNDLE_ID="${BUNDLE_ID:-ai.resonix.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/resonix/resonix/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Resonix/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ResonixBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ResonixGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Resonix"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Resonix"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Resonix"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Resonix" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Resonix/Resources/Resonix.icns" "$APP_ROOT/Contents/Resources/Resonix.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Resonix/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ResonixKit resources"
RESONIXKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ResonixKit_ResonixKit.bundle"
if [ -d "$RESONIXKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ResonixKit_ResonixKit.bundle"
  cp -R "$RESONIXKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ResonixKit_ResonixKit.bundle"
else
  echo "WARN: ResonixKit resource bundle not found at $RESONIXKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Resonix"
killall -q Resonix 2>/dev/null || true
