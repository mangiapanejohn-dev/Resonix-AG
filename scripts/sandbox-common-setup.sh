...
BASE_IMAGE="${BASE_IMAGE:-resonix-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-resonix-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=resonix.sandbox=1)
NOTE
