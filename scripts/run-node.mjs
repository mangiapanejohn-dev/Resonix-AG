...
const logRunner = (message, deps) => {
  if (deps.env.RESONIX_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[resonix] ${message}\n`);
};

const runResonix = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["resonix.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
