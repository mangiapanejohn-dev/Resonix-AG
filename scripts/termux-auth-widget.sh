...
# Resonix Auth Widget for Termux
...
# Check auth status
termux-toast "Checking Resonix auth..."

STATUS=$(ssh "$SERVER" '$HOME/resonix/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/resonix/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|RESONIX_EXPIRING|RESONIXDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/resonix/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|RESONIX_EXPIRED|RESONIXDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Resonix won't work until re-auth"
                ;;
        esac
        ;;
