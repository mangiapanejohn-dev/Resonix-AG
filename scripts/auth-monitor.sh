...
#
# Suggested cron: */30 * * * * /home/admin/resonix/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Resonix notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., resonix-alerts)
...
STATE_FILE="$HOME/.resonix/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Resonix if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use resonix
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Resonix to $NOTIFY_PHONE..."
            resonix send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Resonix is down. Run: ssh l36 '~/resonix/scripts/mobile-reauth.sh'" "urgent"
    exit 1
