...
INSTALL_URL="${RESONIX_INSTALL_URL:-https://resonix.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${RESONIX_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${RESONIX_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="resonix"
PACKAGE_NAME="${RESONIX_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (RESONIX_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${RESONIX_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${RESONIX_INSTALL_LATEST_OUT:-}"
fi
