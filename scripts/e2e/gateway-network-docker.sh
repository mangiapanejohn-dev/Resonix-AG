...
IMAGE_NAME="resonix-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="resonix-net-e2e-$$"
GW_NAME="resonix-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "RESONIX_GATEWAY_TOKEN=$TOKEN" \
	  -e "RESONIX_SKIP_CHANNELS=1" \
	  -e "RESONIX_SKIP_GMAIL_WATCHER=1" \
	  -e "RESONIX_SKIP_CRON=1" \
	  -e "RESONIX_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
