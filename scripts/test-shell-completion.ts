 ...
 * `resonix update`. Use it to verify the completion installation flow
...
\`resonix update\`, \`resonix doctor\`, and \`resonix onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Resonix Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`resonix completion --install\` later to enable.`));
    return;
  }
