...
ZIP=${1:?"Usage: $0 Resonix-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/resonix/resonix/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Resonix-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/resonix/resonix/releases/download/v${VERSION}/"}
