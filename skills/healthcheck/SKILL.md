---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Resonix deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Resonix cron scheduling for periodic checks, or version status checks on a machine running Resonix (laptop, workstation, Pi, VPS).
---

# Resonix Host Hardening

## Overview

Assess and harden the host running Resonix, then align it to a user-defined risk tolerance without breaking access. Use Resonix security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Resonix changes the host firewall, SSH, or OS updates; it does not.
...
5. Resonix gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Resonix can access sensitive data.
...
### 2) Run Resonix security audits (read-only)

As part of the default read-only checks, run `resonix security audit --deep`. Only offer alternatives if the user requests them:

1. `resonix security audit` (faster, non-probing)
2. `resonix security audit --json` (structured output)

Offer to apply Resonix safe defaults (numbered):

1. `resonix security audit --fix`

Be explicit that `--fix` only tightens Resonix defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Resonix version/update status (read-only)

As part of the default read-only checks, run `resonix update status`.
...
- Credential hygiene notes (location of Resonix creds, prefer disk encryption)
...
After Resonix install or first hardening pass, run at least one baseline audit and version check:

- `resonix security audit`
- `resonix security audit --deep`
- `resonix update status`

Ongoing monitoring is recommended. Use the Resonix cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `resonix cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `resonix update status`
...
Before creating, `resonix cron list` and match on exact `name`. If found, `resonix cron edit <id> ...`.
If not found, `resonix cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `resonix update status` (preferred for source checkouts and channels)
2. `npm view resonix version` (published npm version)

## Resonix command accuracy

Use only supported commands and flags:

- `resonix security audit [--deep] [--fix] [--json]`
- `resonix status` / `resonix status --deep`
- `resonix health --json`
- `resonix update status`
- `resonix cron add|list|runs|run`

Do not invent CLI flags or imply Resonix enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Resonix compaction:
