...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `resonix sandbox explain`.
...
Legacy `agent.*` configs are migrated by `resonix doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=resonix-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${RESONIX_STATE_DIR:-$HOME/.resonix}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
