---
summary: "Chrome extension: let Resonix drive your existing Chrome tab"
read_when:
...
The Resonix Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate resonix-managed Chrome profile.
...
Resonix then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Resonix release (npm package) as static files. There is no separate “build” step.

After upgrading Resonix:

- Re-run `resonix browser extension install` to refresh the installed files under your Resonix state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Resonix ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `RESONIX_GATEWAY_TOKEN`)
...
- Open the tab you want Resonix to control.
...
- `ON`: attached; Resonix can drive that tab.
...
Debugging: `resonix sandbox explain`
...
`resonix browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `resonix browser extension install` first to copy the extension to a stable location under your Resonix state directory.
...
- **This is not isolated** like the dedicated resonix-managed profile.
