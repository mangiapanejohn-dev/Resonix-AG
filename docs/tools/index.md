---
summary: "Agent tool surface for Resonix (browser, canvas, nodes, message, cron) replacing legacy `resonix-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `resonix-*` skills
title: "Tools"
---

# Tools (Resonix)

Resonix exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `resonix-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `resonix.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Resonix logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:resonix`: all built-in Resonix tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`resonix node run`).
...
Resonix tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `resonix configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Resonix-managed browser.
...
- Quick smoke: `resonix nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `resonix gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Resonix clamps `tools.sessions.visibility` to `tree`.
