...
- **gateway host** → `resonix` process on the gateway machine
...
`~/.resonix/exec-approvals.json` directly.

CLI: `resonix approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
