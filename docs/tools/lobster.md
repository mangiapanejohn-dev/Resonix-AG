---
title: Lobster
summary: "Typed workflow runtime for Resonix with resumable approval gates."
description: Typed workflow runtime for Resonix — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Resonix run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Resonix runs one Lobster tool call and gets a structured result.
...
Resonix launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | resonix.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
resonix.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Resonix tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Resonix Gateway (see the [Lobster repo](https://github.com/resonix/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Resonix keeps core tools enabled. To restrict core
...
→ resonix calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ resonix calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Resonix tools that do.
