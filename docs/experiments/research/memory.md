---
summary: "Research notes: offline memory system for Resonixd workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.resonix/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Resonix integration
...
Target: Resonixd-style workspace (`agents.defaults.workspace`, default `~/.resonix/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.resonix/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Resonixd to see every session.
...
```
~/.resonix/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Resonix**, but keep a separable core library.

### Why integrate into Resonix?

- Resonix already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `resonix memory recall "…" --k 25 --since 30d`
  - `resonix memory reflect --since 7d`
...
Pragmatic take for `~/.resonix/workspace`:
