...
- `/usage cost` in chats: local cost summary aggregated from Resonix session logs.
- CLI: `resonix status --usage` prints a full per-provider breakdown.
- CLI: `resonix channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
