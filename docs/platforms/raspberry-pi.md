---
summary: "Resonix on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Resonix on a Raspberry Pi
  - Running Resonix on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Resonix on Raspberry Pi

## Goal

Run a persistent, always-on Resonix Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Resonix features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled resonix

# Enable if not
sudo systemctl enable resonix

# Start on boot
sudo systemctl start resonix
```
...
```bash
# Check logs
journalctl -u resonix --no-pager -n 100

# Common fix: rebuild
cd ~/resonix  # if using hackable install
npm run build
sudo systemctl restart resonix
```
