...
- Jobs persist under `~/.resonix/cron/` so restarts don’t lose schedules.
...
resonix cron list
resonix cron run <job-id>
resonix cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.resonix/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `resonix cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Resonix applies a
...
If `delivery` is omitted for isolated jobs, Resonix defaults to `announce`.
...
- Job store: `~/.resonix/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.resonix/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
resonix cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
resonix cron edit <jobId> --agent ops
resonix cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
resonix cron run <jobId>
resonix cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
resonix cron edit <jobId> \
...
```bash
resonix system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`resonix system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `RESONIX_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Resonix applies exponential retry backoff for recurring jobs after consecutive errors:
