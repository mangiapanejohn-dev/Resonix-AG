...
read_when:
  - Setting up Twitch chat integration for Resonix
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Resonix connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
resonix plugins install @resonix/twitch
```

Local checkout (when running from a git repo):

```bash
resonix plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `RESONIX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "resonix", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use RESONIX_TWITCH_ACCESS_TOKEN env var)
...
```bash
RESONIX_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
