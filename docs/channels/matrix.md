...
Matrix is an open, decentralized messaging protocol. Resonix connects as a Matrix **user**
...
```bash
resonix plugins install @resonix/matrix
```

Local checkout (when running from a git repo):

```bash
resonix plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `resonix plugins install @resonix/matrix`
   - From a local checkout: `resonix plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Resonix calls the same
     login endpoint, stores the access token in `~/.resonix/credentials/matrix/credentials.json`,
...
- On first connection, Resonix requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Resonix logs a warning.
...
`~/.resonix/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `resonix pairing list matrix`
  - `resonix pairing approve matrix <CODE>`
...
- On startup, Resonix resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
