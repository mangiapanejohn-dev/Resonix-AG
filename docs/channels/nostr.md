...
read_when:
  - You want Resonix to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Resonix to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`resonix onboard`) and `resonix channels add` list optional channel plugins.
...
```bash
resonix plugins install @resonix/nostr
```

Use a local checkout (dev workflows):

```bash
resonix plugins install --link <path-to-resonix>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "resonix@example.com",
        "lud16": "resonix@example.com"
      }
    }
  }
}
