---
summary: "Mattermost bot setup and Resonix config"
read_when:
...
```bash
resonix plugins install @resonix/mattermost
```

Local checkout (when running from a git repo):

```bash
resonix plugins install ./extensions/mattermost
```
...
4. Configure Resonix and start the gateway.
...
- Approve via:
  - `resonix pairing list mattermost`
  - `resonix pairing approve mattermost <CODE>`
...
Use these target formats with `resonix message send` or cron/webhooks:
