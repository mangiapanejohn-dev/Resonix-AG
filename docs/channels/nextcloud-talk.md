...
```bash
resonix plugins install @resonix/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
resonix plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Resonix" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `resonix pairing list nextcloud-talk`
  - `resonix pairing approve nextcloud-talk <CODE>`
