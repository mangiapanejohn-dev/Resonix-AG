...
```bash
resonix plugins install @resonix/msteams
```

Local checkout (when running from a git repo):

```bash
resonix plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Resonix will offer the local install path automatically.
...
3. Configure Resonix with those credentials.
...
- Talk to Resonix via Teams DMs, group chats, or channels.
...
- On startup, Resonix resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.resonix/resonix.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Resonix, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `resonix-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `resonix plugins install @resonix/msteams`
   - From a local checkout: `resonix plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Resonix in Teams", "full": "Resonix in Teams" },
...
Resonix handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Resonix only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ResonixShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Resonix sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `resonix message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.resonix/msteams-polls.json`.
...
```bash
resonix message send --channel msteams \
...
resonix message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
resonix message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
resonix message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
resonix message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
