...
Tlon is a decentralized messenger built on Urbit. Resonix connects to your Urbit ship and can
...
```bash
resonix plugins install @resonix/tlon
```

Local checkout (when running from a git repo):

```bash
resonix plugins install ./extensions/tlon
```
...
By default, Resonix blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `resonix message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Resonix replies in-thread.
