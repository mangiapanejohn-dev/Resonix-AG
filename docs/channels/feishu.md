...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Resonix to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
resonix plugins install @resonix/feishu
```

Local checkout (when running from a git repo):

```bash
resonix plugins install ./extensions/feishu
```
...
If you just installed Resonix, run the wizard:
...
2. Configuring app credentials in Resonix
...
1. You already ran `resonix channels add` for Feishu
2. The gateway is running (`resonix gateway status`)
...
```bash
resonix pairing approve feishu <CODE>
```
...
2. Run `resonix logs --follow` and look for `chat_id`
...
2. Run `resonix logs --follow` and look for `open_id`
...
| `resonix gateway restart` | Restart gateway service       |
| `resonix logs --follow`   | Tail gateway logs             |
...
4. Check logs: `resonix logs --follow`
...
5. Ensure the gateway is running: `resonix gateway status`
6. Check logs: `resonix logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.resonix/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.resonix/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
