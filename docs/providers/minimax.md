---
summary: "Use MiniMax M2.1 in Resonix"
read_when:
  - You want MiniMax models in Resonix
...
```bash
resonix plugins enable minimax-portal-auth  # skip if already loaded.
resonix gateway restart  # restart if gateway is already running
resonix onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/resonix/resonix/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `resonix.json`:
...
- Use `resonix models list` and `resonix models set minimax/MiniMax-M2.1` to switch.
...
- Running `resonix configure` and selecting **MiniMax M2.1**, or
