---
summary: "Use Venice AI privacy-focused models in Resonix"
read_when:
  - You want privacy-focused inference in Resonix
...
```bash
resonix onboard --auth-choice venice-api-key
```
...
```bash
resonix chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Resonix shows all available Venice models. Pick based on your needs:
...
```bash
resonix models set venice/claude-opus-45
resonix models set venice/llama-3.3-70b
```

List all available models:

```bash
resonix models list | grep venice
```

## Configure via `resonix configure`

1. Run `resonix configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Resonix automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
resonix chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
resonix chat --model venice/claude-opus-45

# Use uncensored model
resonix chat --model venice/venice-uncensored

# Use vision model with image
resonix chat --model venice/qwen3-vl-235b-a22b

# Use coding model
resonix chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `resonix models list` to see currently available models. Some models may be temporarily offline.
