...
你可以在 `~/.resonix/resonix.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/resonix.log"
  }
}
```
...
```bash
resonix channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.resonix/resonix.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/resonix/resonix-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Resonix 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
RESONIX_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `resonix plugins enable diagnostics-otel` 启用插件。
...
- `resonix.tokens`（计数器，属性：`resonix.token`、`resonix.channel`、`resonix.provider`、`resonix.model`）
- `resonix.cost.usd`（计数器，属性：`resonix.channel`、`resonix.provider`、`resonix.model`）
- `resonix.run.duration_ms`（直方图，属性：`resonix.channel`、`resonix.provider`、`resonix.model`）
- `resonix.context.tokens`（直方图，属性：`resonix.context`、`resonix.channel`、`resonix.provider`、`resonix.model`）

消息流：

- `resonix.webhook.received`（计数器，属性：`resonix.channel`、`resonix.webhook`）
- `resonix.webhook.error`（计数器，属性：`resonix.channel`、`resonix.webhook`）
- `resonix.webhook.duration_ms`（直方图，属性：`resonix.channel`、`resonix.webhook`）
- `resonix.message.queued`（计数器，属性：`resonix.channel`、`resonix.source`）
- `resonix.message.processed`（计数器，属性：`resonix.channel`、`resonix.outcome`）
- `resonix.message.duration_ms`（直方图，属性：`resonix.channel`、`resonix.outcome`）

队列 + 会话：

- `resonix.queue.lane.enqueue`（计数器，属性：`resonix.lane`）
- `resonix.queue.lane.dequeue`（计数器，属性：`resonix.lane`）
- `resonix.queue.depth`（直方图，属性：`resonix.lane` 或 `resonix.channel=heartbeat`）
- `resonix.queue.wait_ms`（直方图，属性：`resonix.lane`）
- `resonix.session.state`（计数器，属性：`resonix.state`、`resonix.reason`）
- `resonix.session.stuck`（计数器，属性：`resonix.state`）
- `resonix.session.stuck_age_ms`（直方图，属性：`resonix.state`）
- `resonix.run.attempt`（计数器，属性：`resonix.attempt`）

### 导出的 span（名称 + 关键属性）

- `resonix.model.usage`
  - `resonix.channel`、`resonix.provider`、`resonix.model`
  - `resonix.sessionKey`、`resonix.sessionId`
  - `resonix.tokens.*`（input/output/cache_read/cache_write/total）
- `resonix.webhook.processed`
  - `resonix.channel`、`resonix.webhook`、`resonix.chatId`
- `resonix.webhook.error`
  - `resonix.channel`、`resonix.webhook`、`resonix.chatId`、`resonix.error`
- `resonix.message.processed`
  - `resonix.channel`、`resonix.outcome`、`resonix.chatId`、`resonix.messageId`、`resonix.sessionKey`、`resonix.sessionId`、`resonix.reason`
- `resonix.session.stuck`
  - `resonix.state`、`resonix.ageMs`、`resonix.queueDepth`、`resonix.sessionKey`、`resonix.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `resonix doctor`。
