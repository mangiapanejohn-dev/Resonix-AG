...
summary: 设置指南：在保持最新的同时保持你的 Resonix 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.resonix/workspace`（工作区）+ `~/.resonix/resonix.json`（配置）。
...
- **配置：** `~/.resonix/resonix.json`（JSON/JSON5 格式）
- **工作区：** `~/.resonix/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm resonix setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **Resonix.app**（菜单栏）。
...
- 运行 `resonix setup`，然后 `resonix channels login`，然后手动启动 Gateway 网关（`resonix gateway`）。
...
- **状态存储位置：**
  ...
  - 会话：`~/.resonix/agents/<agentId>/sessions/`
  - 日志：`/tmp/resonix/`
...
- **配对允许列表**：`~/.resonix/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.resonix/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.resonix/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.resonix/workspace` 和 `~/.resonix/` 保持为"你的东西"；不要将个人提示/配置放入 `resonix` 仓库。
