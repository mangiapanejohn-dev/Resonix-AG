---
summary: "CLI reference for `resonix security` (audit and fix common security footguns)"
read_when:
...
```bash
resonix security audit --json | jq '.summary'
resonix security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
resonix security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
