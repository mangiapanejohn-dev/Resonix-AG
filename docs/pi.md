...
This document describes how Resonix integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Resonix uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Resonix directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Resonix's local TUI mode)                                             |
...
├── pi-tools.ts                    # createResonixCodingTools()
...
├── resonix-tools.ts              # Resonix-specific tools
...
2. **Custom Replacements**: Resonix replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Resonix Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
Resonix wraps this with `guardSessionManager()` for tool result safety.
...
Resonix maintains an auth profile store with multiple API keys per provider:
...
Resonix loads custom pi extensions for specialized behavior:
...
Resonix also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Resonix Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Resonix tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.resonix/agents/<agentId>/sessions/` (or `$RESONIX_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-resonix-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-resonix-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-resonix-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-resonix-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
