...
Default label is `ai.resonix.gateway` (or `ai.resonix.<profile>`; legacy `com.resonix.*` may still exist):

```bash
launchctl bootout gui/$UID/ai.resonix.gateway
rm -f ~/Library/LaunchAgents/ai.resonix.gateway.plist
```

If you used a profile, replace the label and plist name with `ai.resonix.<profile>`. Remove any legacy `com.resonix.*` plists if present.
