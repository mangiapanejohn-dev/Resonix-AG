...
Resonix integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Resonix owns lifecycle when `channels.signal.autoStart=true`.
...
- Resonix spawns `imsg rpc` as a child process (legacy iMessage integration).
