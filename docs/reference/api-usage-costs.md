...
Resonix features that can generate provider usage or paid API calls.
...
- `resonix status --usage` and `resonix channels list` show provider **usage windows**
...
Resonix can pick up credentials from:
...
- `resonix status --usage`
- `resonix models status --json`
...
`resonix models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
