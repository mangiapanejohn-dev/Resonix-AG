---
summary: "How Resonix builds prompt context and reports token usage + costs"
read_when:
...
Resonix tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Resonix assembles its own system prompt on every run. It includes:
...
For images, Resonix downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Resonix session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `resonix status --usage` and `resonix channels list` show
...
`cacheWrite`. If pricing is missing, Resonix shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Resonix can
...
Anthropic's 1M context window is currently beta-gated. Resonix can inject the
