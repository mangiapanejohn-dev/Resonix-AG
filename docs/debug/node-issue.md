...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Resonix
...
Running Resonix via Node with `tsx` fails at startup with:

```
[resonix] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `resonix status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch resonix.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node resonix.mjs status` works on Node 25.
