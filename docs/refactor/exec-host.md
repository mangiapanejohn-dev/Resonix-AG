...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.resonix/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.resonix/node.json` (node id + pairing token).
...
Path: `~/.resonix/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.resonix/exec-approvals.sock",
...
- Unix socket at `~/.resonix/exec-approvals.sock` (0600).
