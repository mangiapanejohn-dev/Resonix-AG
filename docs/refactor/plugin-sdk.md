...
- Publish as `resonix/plugin-sdk` (or export from core under `resonix/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ResonixPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ResonixConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ResonixConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ResonixConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ResonixConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ResonixConfig): string;
  };
};
```
...
- Introduce `resonix/plugin-sdk`.
- Add `api.runtime` to `ResonixPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `resonixRuntime: ">=2026.2.0"`).
