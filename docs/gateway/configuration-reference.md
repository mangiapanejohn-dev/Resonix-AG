...
description: "Complete field-by-field reference for ~/.resonix/resonix.json"
---

# Configuration Reference

Every field available in `~/.resonix/resonix.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Resonix uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `resonix doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["resonix-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `resonix plugins install @resonix/mattermost`.
...
Resonix spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@resonix"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `resonix.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Resonix auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Resonix assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.resonix/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.resonix/workspace",
        agentDir: "~/.resonix/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@resonix"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:resonix`   | All built-in tools (excludes provider plugins)                                           |
...
Resonix uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.resonix/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `RESONIX_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `resonix onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `resonix onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `resonix onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `resonix onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `resonix onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `resonix onboard --auth-choice minimax-api`.
...
- Loaded from `~/.resonix/extensions`, `<workspace>/.resonix/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or RESONIX_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `RESONIX_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.resonix-dev` + port `19001`), `--profile <name>` (uses `~/.resonix-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.resonix/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-resonix-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `RESONIX_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or RESONIX_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__resonix__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__resonix__/a2ui/`
...
- Also serves A2UI at `/__resonix__/a2ui/`.
...
- Hostname defaults to `resonix`. Override with `RESONIX_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.resonix/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `resonix dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.resonix/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${RESONIX_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.resonix/credentials/oauth.json`.
...
- Default log file: `/tmp/resonix/resonix-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `resonix doctor --fix` can strip unknown keys).
