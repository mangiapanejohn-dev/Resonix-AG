...
- During heartbeat runs, Resonix treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `resonix sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Resonix skips the heartbeat run entirely (no model call).
...
```bash
resonix system event --text "Check for urgent follow-ups" --mode now
```
