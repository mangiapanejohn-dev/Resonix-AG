...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/resonix`)
...
If the page fails to load, start the Gateway first: `resonix gateway`.
...
you revoke it with `resonix devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.resonix/resonix.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Resonix
...
Resonix **blocks** Control UI connections without device identity.
