...
```bash
resonix tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `resonix tui --deliver`
...
- Check the Gateway logs: `resonix logs --follow`.
- Confirm the agent can run: `resonix status` and `resonix models status`.
...
- No agents in picker: check `resonix agents list` and your routing config.
