...
This document describes how the Resonix ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`resonix acp` exposes an ACP agent over stdio and forwards prompts to a running
Resonix Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `resonix acp` and speaks ACP messages over stdio.
...
`resonix acp` resolves the Gateway URL and auth from CLI flags or config:
